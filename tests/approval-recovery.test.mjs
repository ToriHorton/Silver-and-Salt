...
const env = { ODLA_APP_ID: "silver-and-salt-capital", ODLA_TENANT: "silver-and-salt-capital--dev", ODLA_ENV: "dev", ODLA_RUNTIME: "cory", ODLA_ENDPOINT: "https://db.odla.ai" };
const prod = { ...env, ODLA_TENANT: "silver-and-salt-capital", ODLA_ENV: "prod", ODLA_RUNTIME: "live" };
describe("host approval recovery wiring", () => {
  ...
  it("signs production recovery for the public domain", async () => {
    retry.mockClear();
    await recoverPendingApprovals({}, prod);
    expect(retry).toHaveBeenCalledWith({}, prod, { origin: "https://silverandsaltcapital.com", limit: 100 });
  });
  it("logs and skips instead of rejecting when the scope is unbound", async () => {
    retry.mockClear();
    expect(await recoverPendingApprovals({}, { ...env, ODLA_RUNTIME: "default" })).toBeNull();
  ...
  it.each([{ ODLA_RUNTIME: undefined }, { ODLA_RUNTIME: "default" }, { ODLA_APP_ID: "sibling" }, { ODLA_ENV: "prod" }, { ODLA_TENANT: "silver-and-salt-capital" }])("fails closed for an unbound scope %j", (override) => {
    expect(() => approvalRecoveryOrigin({ ...env, ...override })).toThrow(/scope mismatch/);
  });
});
